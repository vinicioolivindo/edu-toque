        ...
        <View className="px-5 mb-20">
          ...
          <View className="flex-row flex-wrap justify-between">
            {dummyAlbums.map((album, index) => (
              <TouchableOpacity key={index} className="w-[48%] mb-4 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <View className="w-full h-32 bg-[#D1E8E6] items-center justify-center">
                   {/* Simulação da capa do álbum (fundo de caderno do print) */}
                   <Text className="text-gray-400 opacity-50">Capa do Álbum</Text>
                </View>
                
                <View className="p-3 flex-row justify-between items-center bg-white">
                  <View>
                    <Text className="font-bold text-gray-800">{album.title}</Text>
